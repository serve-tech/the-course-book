...
- Organize by feature under `app/features/<feature>/` for pure domain logic and components, `app/routes/` for route modules, and `app/server/` for server-only modules whose filenames end in `.server.ts`. Database schema, migrations and seed data live under `app/db/`.
...
- Every mutation is one database transaction in `app/server/`, keyed on the authenticated user from the request context, never on an id supplied by the form. Multi-row list changes take the per-user advisory lock documented in the architecture guide.
...
- Schema changes ship as Drizzle migration files generated from `app/db/schema.ts` and committed with the change. Never edit an applied migration; never use `drizzle-kit push` outside a throwaway local database.
