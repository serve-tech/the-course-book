...
describe("rounds, membership and personal order", () => {
  it("uses cloud personal ranks over stale local order while retaining round counts", () => {
    const state = {
      ...emptyAccount(),
      myList: ["b", "a"],
      played: { a: 99 },
  ...
  it("appends round-only courses after the cloud-ranked My List", () => {
    const extendedCatalog = new Map([
      ...catalog,
      [
        "c",
        courseSchema.parse({
  ...
  it("preserves local order only when there are no cloud memberships yet", () => {
    expect(
      reconcileAccount(
        {
          ...emptyAccount(),
  ...
  it("does not drop hidden courses during a move", () => {
    ...
    expect(moveCourse(state, "b", 1).myList).toEqual([
      "b",
      "a",
      "hidden",
    ]);
