...
export function moveCourse(
  ...
  return { ...state, myList: order, orderRevision: state.orderRevision + 1 };
}
...
export function reconcileAccount(
  ...
  for (const id of roundCourseIds) played[id] = (played[id] ?? 0) + 1;
  const myList = [...state.myList];
  for (const id of Object.keys(played))
    if (!myList.includes(id)) myList.push(id);
  // Preserve v174's fresh-device ordering: round-backed courses precede memberships.
  if (myList.length) {
    for (const membership of memberships) {
      const duplicate = myList.findIndex(
        (id) =>
          id !== membership.id &&
          equivalentCourses(catalog.get(id), catalog.get(membership.id)),
      );
      if (duplicate >= 0) myList[duplicate] = membership.id;
      else if (!myList.includes(membership.id)) myList.push(membership.id);
    }
  } else {
    for (const membership of [...memberships].sort(
      (a, b) => a.rank - b.rank || a.id.localeCompare(b.id),
    ))
      if (!myList.includes(membership.id)) myList.push(membership.id);
  }
  return {
    ...state,
    played,
